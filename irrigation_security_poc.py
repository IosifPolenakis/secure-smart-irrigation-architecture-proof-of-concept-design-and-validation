...
pass/fail decision" column of the paper.
...
class TelemetryRecord:
    ...
    def canonical_payload(self) -> str:
        return f"{self.device_id}|{self.value}|{self.unit}|{self.measurement_time}|{self.boot_id}|{self.seq}|{self.ttl_seconds}"
...
def build_provo_record(rec: "TelemetryRecord") -> dict:
    ...
    digest = hashlib.sha256(rec.canonical_payload().encode()).hexdigest()
...
class SecureController:
    ...
    def ingest_telemetry(self, rec: TelemetryRecord) -> Tuple[bool, List[str]]:
        ...
        accept = auth_ok and fresh_ok and ordered_ok and quality_ok
        if accept:
...
def scenario_spoofed_telemetry():
    ...
    for ctrl_cls in (NaiveController, SecureController):
        ...
        legit = TelemetryRecord(
            device_id="sensor-023", value=0.31, unit="m3/m3",
            measurement_time=now, gateway_receive_time=now,
            boot_id="boot-6f2c8f20", seq=4821, ttl_seconds=300, signature="",
        )
        legit.signature = hmac_sign(legit_key, legit.canonical_payload())
        ...
        fake = TelemetryRecord(
            ...
            seq=1,
            ttl_seconds=300,
            signature=hmac_sign(UNKNOWN_KEY, "irrelevant"),  # wrong key
        )
...
def run_all():
    ...
    print("-" * len(header))
    print(f"Unsafe outcomes -> naive: {naive_unsafe}/{len(by_scenario)}   "
          f"secure: {secure_unsafe}/{len(by_scenario)}")
    ...
    print("bounds every scenario, matching the 'Expected architectural response'")
    print("column of Table 6.")
    ...
    # secure: 0/6 unsafe") a property that is checked by the program
    # itself, not merely asserted in prose. A non-zero exit code means
    # the claimed result was NOT reproduced on this run/environment.
    assert len(by_scenario) == 6, "expected exactly 6 Table 6 scenarios"
    assert naive_unsafe == 6, f"expected naive controller unsafe on all 6 scenarios, got {naive_unsafe}"
    assert secure_unsafe == 0, f"expected secure controller unsafe on 0 scenarios, got {secure_unsafe}"
    print("\nAll assertions passed: naive=6/6 unsafe, secure=0/6 unsafe "
          "(exit code 0 confirms this run reproduced the claimed result).")
